...
class _Signal {
  ...
  const _Signal({
    ...
    required this.fundingLevel,
    required this.oiChange,
    required this.oiLevel,
    required this.lsRatio,
    required this.lsLevel,
    required this.liqWall,
    required this.liqSide,
...
const _signals = {
  'BTC': _Signal(
    ...
    riskReward: '2.8:1',
    reasoning:
        'Funding at 0.071% signals overleveraged longs. OI rising into resistance with no breakout confirmation. Large liquidation cluster below at \$94,200 could cause a cascade. Wait for a flush below \$95,000 before entering long.',
  ),
  'ETH': _Signal(
    coin: 'ETH',
    price: '\$3,842',
    ...
    riskReward: '2.4:1',
    reasoning:
        'Neutral funding with declining OI suggests deleveraging is healthy. Long/short ratio near 1.2 means balanced positioning. Spot buying pressure visible. Strong support at \$3,750. Risk/reward favors longs.',
  ),
  'SOL': _Signal(
    coin: 'SOL',
    price: '\$184',
    funding: '-0.012%',
    fundingLevel: 'Slightly Negative',
    oiChange: '+5% (6h)',
    oiLevel: 'Rising',
    lsRatio: '0.89',
    lsLevel: 'Short-Heavy',
    ...
    riskReward: '3.1:1',
    reasoning:
        'Negative funding + short-heavy positioning creates conditions for a short squeeze. Large liquidation wall at \$195 will accelerate the move. OI rising with price suggests new money entering. High-conviction long setup.',
  ),
  'BNB': _Signal(
    coin: 'BNB',
    price: '\$612',
    funding: '+0.051%',
    fundingLevel: 'Elevated',
    oiChange: '+8% (6h)',
    oiLevel: 'Rising',
    lsRatio: '1.54',
    lsLevel: 'Long-Biased',
    ...
    riskReward: '2.2:1',
    reasoning:
        'Elevated funding with rising OI into resistance. Long-biased L/S ratio exposes downside risk. Potential flush to \$588 liquidation wall before reversal. Watch for spot buying at \$590 before entering.',
  ),
  'XRP': _Signal(
    coin: 'XRP',
    price: '\$2.14',
    ...
    riskReward: '2.0:1',
    reasoning:
        'Low funding with stable OI. No strong directional bias. Price consolidating in a range. Await breakout confirmation above \$2.25 or breakdown below \$2.05 before committing.',
  ),
  'DOGE': _Signal(
    coin: 'DOGE',
    price: '\$0.182',
    ...
    oiLevel: 'Surging',
    lsRatio: '2.14',
    lsLevel: 'Extremely Crowded Longs',
    liqWall: '\$0.155',
    liqSide: 'Below',
    sentiment: 85,
    ...
    riskReward: 'N/A',
    reasoning:
        'Funding rate at 0.098% — one of the highest levels seen this cycle. OI surging +24% in 6 hours is unsustainable. L/S ratio of 2.14 means extreme crowding. High probability of violent liquidation cascade to \$0.155. Do not chase.',
  ),
};
...
class _TradeNowScreenState extends ConsumerState<TradeNowScreen> {
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('$_selectedCoin signal coming soon',
                ...
                const Text(
                    'Live AI analysis for this coin is not yet available.',
                    style: TextStyle(fontSize: 12, color: AppColors.textMuted)),
              ],
            ),
          ),
  ...
  Widget _buildMetricsGrid(_Signal s) {
    final fundingAsync = ref.watch(liveFundingProvider);
    final liveFundingRates = fundingAsync.value ?? const [];

    LiveFundingRate? liveFunding;
    for (final f in liveFundingRates) {
      if (f.symbol == '${s.coin}USDT') {
        liveFunding = f;
        break;
      }
    }

    final String fundingVal;
    final String fundingLvl;
    final Color fundingColor;

    if (liveFunding != null) {
      fundingVal = liveFunding.formatted;
      if (liveFunding.rate.abs() >= 0.0005) {
        fundingLvl = 'Very High';
        fundingColor = AppColors.brandRed;
      } else if (liveFunding.rate.abs() >= 0.0003) {
        fundingLvl = 'High';
        fundingColor = AppColors.brandRed;
      } else if (liveFunding.rate.abs() >= 0.00015) {
        fundingLvl = 'Elevated';
        fundingColor = AppColors.brandAmber;
      } else {
        fundingLvl = 'Neutral';
        fundingColor = AppColors.brandGreen;
      }
    } else {
      fundingVal = s.funding;
      fundingLvl = s.fundingLevel;
      fundingColor = s.fundingLevel == 'High' || s.fundingLevel == 'Very High'
          ? AppColors.brandRed
        ...
        Row(
          children: [
            Expanded(
              ...
              value: s.lsRatio,
              badge: s.lsLevel,
              badgeColor: s.lsLevel.contains('Crowded') ||
                      s.lsLevel == 'Extremely Crowded Longs'
                  ? AppColors.brandRed
  ...
  Widget _buildHistoricalSetups() {
    const setups = [
      _HistoricalSetup(
          'BTC · Mar 2024',
          'Similar funding spike to 0.08% before flush to \$59K, then rip to \$73K',
          '+24%',
          true),
      _HistoricalSetup(
          'ETH · Nov 2023',
          'Negative funding + short squeeze setup resolved with +18% gain in 5 days',
          '+18%',
          true),
      _HistoricalSetup(
          'SOL · Jan 2024',
          'Crowded longs + high OI led to -22% liquidation cascade',
          '-22%',
          false),
    ];
...
class _HistoricalSetup {
  ...
  final bool positive;
  const _HistoricalSetup(
      this.title, this.description, this.outcome, this.positive);
}
...
class _LivePriceText extends ConsumerStatefulWidget {
  final String symbol;
  ...
  @override
  ConsumerState<_LivePriceText> createState() => _LivePriceTextState();
}

class _LivePriceTextState extends ConsumerState<_LivePriceText> {
  Color? _flashColor;
  Timer? _timer;
  double? _prevPrice;
  ...
  Widget build(BuildContext context) {
    ...
    final ticker = tickers[widget.symbol];
    ...
    if (ticker != null) {
      ...
      if (_prevPrice != null && _prevPrice != currentPrice) {
        _timer?.cancel();
        setState(() {
          _flashColor = currentPrice > _prevPrice! ? AppColors.brandGreen : AppColors.brandRed;
        });
      ...
      return AnimatedDefaultTextStyle(
        ...
        style: widget.style.copyWith(
          color: _flashColor ?? widget.style.color ?? Colors.white,
        ),
        child: Text(priceStr),
      );
    }

    return Text(widget.fallbackPrice, style: widget.style);
  }
}
